...
Giao diện bệnh nhân chỉ hiển thị một nhóm gồm ba chiến lược sắp lịch trình.
...
| `finish_early` | Ưu tiên thời gian vào khám | Đưa bệnh nhân vào các phòng có thể tiếp nhận sớm; thời gian chờ bác sĩ có thể dài hơn |
| `leave_fast` | Ưu tiên kết quả đến tay bác sĩ | Sắp xếp để bác sĩ có đủ các kết quả bắt buộc trong thời gian sớm nhất |

#### FR-PREF-002: Tiêu chí nội bộ

Trường `route_priority` không hiển thị cho bệnh nhân trong bản demo và luôn dùng giá trị trung lập `system`. Nhu cầu hỗ trợ di chuyển được thu thập ở luồng hỗ trợ riêng, không phải một chiến lược sắp lịch trình.

#### FR-PREF-003: Tính lại khi thay đổi

Khi bệnh nhân đổi một trong ba chiến lược, hệ thống phải tạo hoặc sắp xếp lại phương án. Không chỉ thay nhãn hiển thị.
...
→ Chọn Ưu tiên thời gian vào khám
→ Nhận 3 phương án
